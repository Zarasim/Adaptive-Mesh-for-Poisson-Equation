...
\begin{enumerate}
    ...
    \item Set the parameter $B = 1 - \gamma$ and enforce the condition $m = A + Bl_{i,j}^{-2\gamma} = 1$. This ensures that the new mesh boundary matches with the original boundary region. 
...
\T{}$. Given the corresponding Jacobian $\vec{J}_{K}$ with eigenvalues
...
\begin{equation}
\vec J =  \frac{\gamma}{1-\gamma} (\xi^{2} + \eta^{2})^{\frac{\gamma}{2(1-\gamma)-1}} \begin{bmatrix}
 \xi^{2} + (\xi^{2} + \eta^{2})\frac{1-\gamma}{\gamma} &  \xi \eta  \\
 \xi \eta &  \eta^{2} + (\xi^{2} + \eta^{2})\frac{1-\gamma}{\gamma}.    
\end{bmatrix} 
\end{equation}

The eigenvalues are $\lambda_{1} = (\frac{1-\gamma}{\gamma})^{2}$ and $\lambda_{2} = 1/\gamma^{2}$. Finally, the skewness is expressed in terms of the interior angle $\omega$ by:

\begin{equation}
 Q(\omega) = \frac{1}{2}\left(\frac{\lambda_{1}}{\lambda_{2}} + \frac{\lambda_{2}}{\lambda_{1}}\right) =  \frac{1}{2}\left((1-\gamma)^{2} + \frac{1}{(1-\gamma)^{2}}\right) =  \frac{1}{2}\left(\left(\frac{\pi}{2\omega}\right)^{2} + \left(\frac{2\omega}{\pi}\right)^{2}\right).
\end{equation}

We can further simplify the expression by writing $w = x \pi$, with $1 < x < 2$:

\begin{equation}
 Q(x) = \frac{1 + 16x^{4}}{8x^{2}}, \quad x \in (1,2).
\end{equation}
...
\hfill
\subfloat[{\label{fig:3-b}} The shape regularity constant plotted against the dimension of the Galerkin space for different meshes.]
{
\begin{tikzpicture}
  \begin{axis}[
      cycle list/Dark2,
