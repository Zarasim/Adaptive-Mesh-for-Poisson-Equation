...
In this chapter, we treat the Poisson equation defined on the L-shaped domain with prescribed Dirichlet boundary conditions. The numerical solution of the Poisson equation on non-convex domains has been already addressed in \cite{CKW:2002,SH:2016,ZS:2002}. The spatial discretisation is performed with the discontinuous Galerkin (dG) method, introduced in \cite{Reed:1973} for neutron transport problems. The method has been then extended to solve first order hyperbolic systems and to general convection-diffusion problems \cite{Cockburn:1998,Cockburn:2001}.
...
\begin{figure}
  \subfloat[{\label{fig:1-a}}
    Solution error as a function of iterations of the MMPDE with gradient monitor function.
  ]{
\begin{tikzpicture}
  \begin{axis}[
      cycle list/Dark2,
...
\subfloat[{\label{fig:1-b}}
  Solution error as a function of iterations of the MMPDE with curvature monitor function.
]
{
\begin{tikzpicture}
  \begin{axis}[
...
\begin{figure}
  \subfloat[{\label{fig:1-c}}
    Solution error as a function of iterations of the MMPDE with a-posteriori monitor function ($\hat{\beta} = 0.0$).
  ]{
\begin{tikzpicture}
  \begin{axis}[
      cycle list/Dark2,
...
\subfloat[{\label{fig:1-d}}
    Solution error as a function of iterations of the MMPDE with a-posteriori monitor function ($\hat{\beta} = 0.33$).
]
{
\begin{tikzpicture}
  \begin{axis}[
...
\subfloat[{\label{fig:1-e}}
    Solution error as a function of iterations of the MMPDE with a-posteriori monitor function ($\hat{\beta} = 0.5$).
]
{
\begin{tikzpicture}
  \begin{axis}[
...
\subfloat[{\label{fig:1-f}}
    Solution error as a function of iterations of the MMPDE with a-posteriori monitor function ($\hat{\beta} = 0.99$).
]
{
\begin{tikzpicture}
  \begin{axis}[
...
We start by assuming that the interpolation error of $u_{h}(r)$ is strongly depdendent on $r$ and weakly depdendent on $\theta$, so that $u_{h}(r) \sim  h^{2}\frac{\d^{2}}{\d r^{2}}u$, where $h$ is the local mesh size of an element close to the corner and $r$ is the distance from the origin. The error is dominated by $E_{max} =  h^{2} r^{2/3 - 2}$. Following the \textit{equidistribution principle}, we seek to equidistribute $E_{max}$, so that we obtain $h \sim r^{2/3}$. Given the computation variable $s$, which represents the distance from the origin in the computational domain $\W_{c} \equiv \W$, we can interpret $h$ as being proportional to $\d r/ \d s$. Solving the differential equation $\frac{\d r}{\d s} = h = r^{2/3}$ we obtain that $r \sim s^{3}$.


\paragraph{Solution of Monge-Ampère equation}

We now look for a radially symmetric transformation from the corner region to itself. More details and extensions of this application can be found in \cite{BRW:2015}. Locally, the first integral of the Monge-Ampère equation implies that $r$ satisfies the Ordinary Differential Equation (ODE) in polar coordinates:

\begin{equation}
    m(r) \, r \d r \d \theta = s \d s \d \theta,
\label{eq:Monge-Ampere}
\end{equation}

where $m(r) = r^{\gamma}$ is the monitor function, which will be determined by the \textit{a-priori} estimate for the interpolation error:

$$r^{\gamma} r r^{2/3} = r^{1/3},$$

so that $\gamma = -4/3$.

We expect that for large $r$ the adapted mesh is almost regular, so that $m(r) \sim 1$ far from the corner, and consider the general expression 

\begin{equation}
    m(r) = \alpha + \beta r^{-4/3}, \quad \alpha,\beta \in \reals,
\end{equation}

where $\alpha$ and $\beta$ are two regularisation parameters.

From \eqref{eq:Monge-Ampere} we integrate over the variable $\theta$ and obtain

$$\left(\alpha + \beta r^{-4/3}\right) r \frac{\d r}{\d s} = s,$$

so that $r$ satisfies the nonlinear algebraic equation 

\begin{equation}
    \alpha r^{2} + 3 \beta r^{2/3} = s^{2}.
\label{eq:mesh_transformation}
\end{equation}

The equation \eqref{eq:mesh_transformation} then determines the mesh transformation. Note that the parameter $\alpha$ and $\beta$ controls the level of mesh compression near the corner. In fact, for high values of $\beta$ we have $r \sim s^{3}$, whereas for large $\alpha$ we have $r \sim s$. For the remainder of the analysis, we choose $\beta = 1/3$, as this is the maximum admissible value that satisfies the boundary condition $\alpha + 3 \beta = 1$ in equation \eqref{eq:mesh_transformation}, when $r,s = 1$. 

\paragraph{Computation of the OT mesh}

Suppose we have a uniform regular mesh $(\xi_{i,j},\eta_{i,j})$ in $\W_{c}$ that we want to map to an adapted non-uniform mesh $(x_{i,j},y_{i,j})$ in the physical domain, with $i,j = 1,\dots,N$. The application of the Monge-Ampère method provides the desired mesh as follows:
...
\begin{enumerate}
    ...
    \item Compute the length $l_{i,j}$ of the straight line spanned from the origin with angle $\theta_{i,j}$ to the boundary of the L-shaped domain.
    \item Set $\beta = 1/3$ and enforce the condition $m = \alpha + \beta l_{i,j}^{-4/3} = 1$, which leads to $ \alpha = 1 -  3 \beta l_{i,j}^{-4/3}$. This ensures that the new mesh boundaries match the L-shaped ones.
...
\begin{figure}[h!]
    ...
    \includegraphics[width=0.4\linewidth]{Images/Test1/OT/u_OT.png}
  }
     \subfloat[\label{fig:u_OT_zoom} Zoom of the solution in the re-entrant corner.]{
    \includegraphics[width=0.4\linewidth]{Images/Test1/OT/u_OT_zoom.png}
  }
\caption{Solution of eq.\eqref{eq:poisson_corner} on the OT mesh.}  
\label{fig:OT_mesh}
\end{figure}
...
\addplot+[color=green, mark = diamond]
	table[x=dof,y=error, col sep=comma]{Data/Test1/OT/OT_ref.csv};
\addlegendentry{OT mesh};
\addplot+[color=cyan, mark = star]
	table[x=dof,y=error, col sep=comma]{Data/Test1/h-adaptive/MS/href_0.99.csv};
\addlegendentry{h-ref ($\beta = 0.0$)};
\addplot+[color=orange, mark = square]
	table[x=dof,y=error, col sep=comma]{Data/Test1/r-adaptive/a-posteriori/beta_0.99/posteriori_rate.csv};
\addlegendentry{Winslow' ($\beta = 0.0$)};
\draw[thick,black] (30000,0.00002) -- (80000,0.00002) -- (30000,0.000053)--cycle;
\node at (40000,0.000028) {$1.0$};
\draw[thick,black] (30000,0.000448083296781) -- (100000,0.000448083296781) -- (100000,0.0002)--cycle;
\node at (70000,0.00035) {$0.67$};
\end{axis}
\end{tikzpicture}
\caption{Asymptotic convergence rates for different adaptive mesh strategies. We observe that the OT mesh achieves the best accuracy in the family of $r$-adaptive methods and is almost as accurate as the \textit{ms} $h$-refinement method.}
\end{figure}


\subsection{Quality of the mesh}
...
\begin{figure}
  \subfloat[{\label{fig:1-a}}
    The mesh condition plotted against the dimension of the Galerkin space for different meshes. 
  ]{
\begin{tikzpicture}
  \begin{axis}[
      cycle list/Dark2,
      ...
      ymode=log,
      xlabel=$\dim \fes$,
      ylabel = $\Norm{\jump{h}/\avg{h}}_{\leb{\infty}_{\cE_{I}}}$,
      grid=both,
      minor grid style={gray!25},
      major grid style={gray!25},
      width=0.5\linewidth,
...
\addplot+[color = cyan, mark=star]
	table[x=dof,y=q, col sep=comma]{Data/Test1/h-adaptive/MS/q_0.0.csv};
\addlegendentry{h-ref ($\beta = 0.0$)};
\addplot+[color = green, mark=diamond]
	table[x=dof,y=q, col sep=comma]{Data/Test1/OT/q.csv};
\addlegendentry{OT mesh};
\addplot+[color = orange, mark=square]
    table[x=dof,y=q, col sep=comma]{Data/Test1/r-adaptive/a-posteriori/beta_0.0/q.csv};
\addlegendentry{Winslow' ($\beta = 0.0$)};
\addplot+[color = black, mark=triangle]
	table[x=dof,y=q, col sep=comma]{Data/Test1/uniform/q.csv};
\addlegendentry{unstructured};
\end{axis}
\end{tikzpicture}
}
\hfill
\subfloat[{\label{fig:1-b}} The shape regularity constant plotted against the dimension of the Galerkin space for different meshes.]
{
\begin{tikzpicture}
  \begin{axis}[
      cycle list/Dark2,
...
\addplot+[color = cyan, mark=star]
	table[x=dof,y=mu, col sep=comma]{Data/Test1/h-adaptive/MS/mu_0.0.csv};
%\addlegendentry{h-refinement};
\addplot+[color = green, mark=diamond]
	table[x=dof,y=mu, col sep=comma]{Data/Test1/OT/mu.csv};
%\addlegendentry{OT mesh};
\addplot+[color = orange, mark=square]
    table[x=dof,y=mu, col sep=comma]{Data/Test1/r-adaptive/a-posteriori/beta_0.0/mu.csv};
%\addlegendentry{Winslow' method};   
\addplot+[color = black, mark=triangle]
	table[x=dof,y=mu, col sep=comma]{Data/Test1/uniform/mu.csv};
%\addlegendentry{unstructured};
\end{axis}
\end{tikzpicture}}
\caption{Here we observe that the mesh condition is optimal for the h-refinement and  does not depend on $\dim \fes$. The Winslow's method performs poorly both in terms of mesh condition and shape regularity. The OT mesh condition does not depend on $\dim \fes$ but is not optimal, as visible in Fig.\ref{fig:u_OT_zoom}; however the OT strategy provides highest shape regularity.}
\end{figure}
...
Numerically, $\omega$ has been set to $2\pi - \epsilon$ with $\epsilon = 10^{-3}$. The value of $\beta$ must be selected within the range $(1/2,1]$.
...
\addlegendentry{unstructured};
\addplot+[color=cyan, mark = star]
	table[x=dof,y=error, col sep=comma]{Data/Test2/h-adaptive/href_0.0.csv};
\addlegendentry{h-ref ($\beta: $ 0.0)};
\addplot+[color=red, mark = star]
	table[x=dof,y=error, col sep=comma]{Data/Test2/h-adaptive/href_0.99.csv};
\addlegendentry{h-ref ($\beta: $ 0.99)};
\addplot+[color=green, mark = diamond]
table[x=dof,y=error, col sep=comma]{Data/Test2/OT_ref/OT_ref.csv};
\addlegendentry{OT};
\addplot+[color=orange, mark = square]
	table[x=dof,y=error, col sep=comma]{Data/Test2/r-adaptive/a-posteriori/beta_0.99/posteriori.csv};
\addlegendentry{Winslow' ($\beta: $ 0.99)};
\draw[thick,black] (100000,0.00001) -- (300000,0.00001) -- (100000,0.00003)--cycle;
\node at (130000,0.000015) {$1.0$};
\draw[thick,black] (100000,0.0008660254) -- (300000,0.0008660254) -- (300000,0.0005)--cycle;
\node at (220000,0.0007) {$0.5$};
\end{axis}
\end{tikzpicture}
\caption{Asymptotic convergence rates for different adaptive strategies on $\fes$. Note that the rate of convergence is optimal for h-refinement and the OT strategy. Uniform mesh refinement yields the expected convergence rate of 0.5. }
\end{figure}
...
\addplot+[color = red, mark=star]
	table[x=dof,y=q, col sep=comma]{Data/Test2/h-adaptive/q_0.0.csv};
%\addlegendentry{h-refinement ($\beta = 0$)};
\addplot+[color = green, mark=diamond]
	table[x=dof,y=q, col sep=comma]{Data/Test2/OT_ref/q.csv};
%\addlegendentry{OT mesh};
\addplot+[color = orange, mark=square]
    table[x=dof,y=q, col sep=comma]{Data/Test2/r-adaptive/a-posteriori/beta_0.99/q.csv};
%\addlegendentry{Winslow' (a-posteriori)};
\addplot+[color = black, mark=triangle]
	table[x=dof,y=q, col sep=comma]{Data/Test2/uniform/q.csv};
%\addlegendentry{uniform unstructured};
\end{axis}
\end{tikzpicture}
}
\hfill
...
\addplot+[color = red, mark=star]
	table[x=dof,y=mu, col sep=comma]{Data/Test2/h-adaptive/mu_0.0.csv};
%\addlegendentry{h-refinement};
\addplot+[color = green, mark=diamond]
	table[x=dof,y=mu, col sep=comma]{Data/Test2/OT_ref/mu.csv};
%\addlegendentry{OT mesh};
\addplot+[color = orange, mark=square]
table[x=dof,y=mu, col sep=comma]{Data/Test2/r-adaptive/a-posteriori/beta_0.99/mu.csv};
%\addlegendentry{Winslow' method};   
\addplot+[color = black, mark=triangle]
	table[x=dof,y=mu, col sep=comma]{Data/Test2/uniform/mu.csv};
%\addlegendentry{uniform unstructured};
\end{axis}
\end{tikzpicture}}
\end{figure}
...
\begin{figure}[h!]
\centering
\subfloat[{\label{fig:crack_h_ref-0.0}}
    h-refinement on re-entrant corner for $\beta=  0.0$
  ]{
    \includegraphics[scale=0.2]{Images/Test2/h-adaptive/u_0.0.png}
}
\subfloat[{\label{fig:crack_h_ref-0.99}}
    h-refinement on re-entrant corner for $\beta =0.99$ 
  ]{
    \includegraphics[scale=0.2]{Images/Test2/h-adaptive/u_0.99.png}
}\\
\centering
\subfloat[{\label{fig:Winslow posteriori}}
    Winslow' method with a-posteriori monitor function $\beta = 0.99$
 ]{
    \includegraphics[scale=0.25]{Images/Test2/r-adaptive/u_0.99_11862.png}
}
\subfloat[{\label{fig:Winslow_computational_domain}}
    Computational domain for Winslow's method ]{
    \includegraphics[scale=0.25]{Images/Test2/r-adaptive/u_c.png}
}\\
\subfloat[{\label{fig:crack_OT}}
    OT mesh
  ]{
    \includegraphics[scale=0.2]{Images/Test2/r-adaptive/OT.png}
}
\subfloat[{\label{fig:crack_OT_zoom}}
    Zoom of OT mesh near the corner
  ]{
    \includegraphics[scale=0.2]{Images/Test2/r-adaptive/OT_zoom.png}
}
\label{fig:crack}
\end{figure}
...
In this Chapter, we have analysed different adaptive mesh methods for the solution of the Poisson equation on the L-shaped domain. We first derived its SIP-dG variational form using the theoretical framework of the weighted Sobolev spaces. We stated the Theorem of existence and uniqueness of the solution, provided that a weight $\beta$ is greater than a specif threshold, based on the geometry of the non-convex domain. 
...
An \textit{Optimal Transport} mesh has also been devised, inspired by the strong dependence of the solution on the radial variable near the re-entrant corner. This method is more accurate than Winslow's strategy and provides a better mesh quality. Furthermore, the computational cost is much cheaper as does not require the numerical solution of a PDE with a semi-implicit time discretisation. 

Finally, we have assessed the quality of the resulting meshes in terms of \textit{mesh condition} and \textit{shape regularity}. While the h-refinement strategy achieves lowest mesh condition, the OT mesh provides highest shape regularity. On the contrary, the quality of the mesh generated by Winslow's method is very poor, as the mesh condition increases and the shape regularity decreases dramatically as a function of the dimension of the Galerkin space.
