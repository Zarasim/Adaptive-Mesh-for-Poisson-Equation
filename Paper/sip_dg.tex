...
\addplot+[color=cyan, mark = triangle]
	table[x=dof,y=Q, col sep=comma]{Data/Test1/OT/skewness_crack.csv};
\addlegendentry{$\omega = 2 \pi$};
%\addplot+[color=cyan, mark = star,dotted]
%table[x=dof,y=Q_theory, col sep=comma]{Data/Test1/OT/skewness_crack.csv};
%\addlegendentry{};
\end{axis}
\end{tikzpicture}
...
\end{axis}
\end{tikzpicture}}
\caption{Here we observe that the mesh condition is optimal for the h-refinement and  does not depend on $\dim \fes$ for both domains. The OT mesh condition does not depend on $\dim \fes$ but is not optimal, as visible in Fig.\ref{fig:u_OT_zoom}; however the OT strategy provides highest shape regularity. The Winslow's method performs poorly both in terms of mesh condition and shape regularity in both cases.}
\end{figure}
...
Finally, we have compared the quality of all the adaptive meshes in terms of \textit{mesh condition} and \textit{shape regularity}. With regard to the L-shaped domain, the h-refinement strategy achieves lowest mesh condition, whereas the OT mesh yields higher shape regularity. Concerning the crack domain, the shape regularity of OT mesh decreases and is inferior to the h-adaptive one, as suggested by the analogy with the \textit{skewness} quantity obtained in \S \ref{sec:skewness}. On the contrary, the quality of the  Winslow's mesh is very poor and decreases dramatically as a function of the dimension of the Galerkin space.
