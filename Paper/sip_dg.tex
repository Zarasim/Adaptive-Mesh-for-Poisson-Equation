...
\abstract{In this paper, we investigate different h- and r-adaptive mesh strategies for the numerical solution
  of the Poisson equation on non-convex domains. We first discretise the problem with the symmetric interior penalty discontinuous Galerkin method. Then, using the theoretical framework of the weighted Sobolev spaces and the duality argument, we derive an \textit{a-posteriori} estimator of the primal solution in the $\leb{2}$ norm. An $\leb{\infty}$ a-posteriori bound will be also employed for driving the mesh adaptation with the two strategies. While h-refinement always yields optimal order of convergence,  we evidence that only a moving mesh based on Optimal transport is able to reproduce similar results, and outperforms other r-adaptive methods based on a Variational formulation, both in terms of accuracy and quality of the resuting mesh. We will also show that the OT mesh providing the highest accuracy equidistributes the a-posteriori bound for the two norms. Finally, we investigate the properties of the mesh skewness and prove that for an OT mesh this quantity is independendent on the resolution of the mesh. All the results are corroborated by numerical experiments.

\section{Introduction}

Solutions of elliptic boundary value problems defined on non-convex domains display singular behaviour near re-entrant corners. This occurs even when data of the underlying problem are very smooth. Such singular behaviour affects the accuracy of Finite Element Methods (FEMs) throughout the whole domain. Hence, the solutions of the corresponding problems resiude typically only in Sobolev spaces $\sobh{k}$ for small $k>1$ \cite{BG:1988,BG:1989}. The theoretical framework of weighted Sobolev spaces, which were originally studied in \cite{BR:1972,BKP:1979} for elasticity and potential problems, are able to handle such setting and will be employed throughout the paper for discretising the Poisson equation and deriving an a-posteriori bound in the $\leb{2}$ norm. \\
...
\draw[dotted,black] (30000,0.000385859365913) -- (80000,0.0002);
...
\begin{figure}[h!]
    ...
    \includegraphics[width=0.38\linewidth]{Images/Test1/h-adaptive/u_0_0.png}
  }
     \subfloat[\label{fig:beta_0.33} $\hat{\beta} = 0.33$.]{
    \includegraphics[width=0.38\linewidth]{Images/Test1/h-adaptive/u_0_33.png}
  }\\
      \subfloat[\label{fig:beta_0.5} $\hat{\beta} = 0.5$.]{
    \includegraphics[width=0.38\linewidth]{Images/Test1/h-adaptive/u_0_5.png}
  }
 \subfloat[\label{fig:beta_0.99} $\hat{\beta} = 0.99$.]{
    \includegraphics[width=0.38\linewidth]{Images/Test1/h-adaptive/u_0_99.png}
  }
\caption{Solution of eq.\eqref{eq:poisson_corner} near the origin with $\leb{2}$ a-posteriori bound after 15 refinements.}
\label{fig:h-ref_L2}
\end{figure}
...
\addplot+[color=green,mark = triangle]
	table[x=dof,y=error, col sep=comma]{Data/Test1/OT/a_priori/errorL2_0_33.csv};
...
\begin{figure}[h!]
    \centering
      \subfloat[\label{fig:u_OT} OT Mesh.]{
    \includegraphics[width=0.3\linewidth]{Images/Test1/OT/u_OT.png}
  }
     \subfloat[\label{fig:u_OT_zoom} Zoom of the mesh in the re-entrant corner.]{
    \includegraphics[width=0.287\linewidth]{Images/Test1/OT/u_OT_zoom.png}
  }
...
gence is optimal for h-refinement and the OT strategy $(\gamma = 0.53/0.67 in \leb{2} /\leb{\infty} )$ norm.}
\end{figure}
...
\addplot+[color=black, mark = triangle]
table[x=dist,y=measure, col sep=comma]{Data/Test1/OT/measure_L2/measure_L2_0_47.csv};
\addlegendentry{$\gamma = 0.47$}; 
\addplot+[color=red, mark = triangle]
table[x=dist,y=measure, col sep=comma]{Data/Test1/OT/measure_L2/measure_L2_0_53.csv};
\addlegendentry{$\gamma = 0.53$}; 
\addplot+[color=green, mark = triangle]
table[x=dist,y=measure, col sep=comma]{Data/Test1/OT/measure_L2/measure_L2_0_59.csv};
\addlegendentry{$\gamma = 0.59$}; 
\addplot+[color=blue, mark = triangle]
table[x=dist,y=measure, col sep=comma]{Data/Test1/OT/measure_L2/measure_L2_0_65.csv};
\addlegendentry{$\gamma = 0.65$}; 
\addplot+[color=cyan, mark = triangle]
table[x=dist,y=measure, col sep=comma]{Data/Test1/OT/measure_L2/measure_L2_0_71.csv};
\addlegendentry{$\gamma = 0.71$}; 
\addplot+[color=magenta, mark = triangle]
table[x=dist,y=measure, col sep=comma]{Data/Test1/OT/measure_L2/measure_L2_0_78.csv};
\addlegendentry{$\gamma = 0.78$}; 
\addplot+[color=orange, mark = triangle]
table[x=dist,y=measure, col sep=comma]{Data/Test1/OT/measure_L2/measure_L2_0_84.csv};
...
\addplot+[color=black, mark = triangle]
table[x=dist,y=measure, col sep=comma]{Data/Test1/OT/measure_Linfty/measure_Linfty_0_62.csv};
\addlegendentry{$\gamma = 0.62$}; 
\addplot+[color=red, mark = triangle]
table[x=dist,y=measure, col sep=comma]{Data/Test1/OT/measure_Linfty/measure_Linfty_0_65.csv};
\addlegendentry{$\gamma = 0.65$}; 
\addplot+[color=green, mark = triangle]
table[x=dist,y=measure, col sep=comma]{Data/Test1/OT/measure_Linfty/measure_Linfty_0_67.csv};
\addlegendentry{$\gamma = 0.67$}; 
\addplot+[color=blue, mark = triangle]
table[x=dist,y=measure, col sep=comma]{Data/Test1/OT/measure_Linfty/measure_Linfty_0_71.csv};
\addlegendentry{$\gamma = 0.71$}; 
\addplot+[color=cyan, mark = triangle]
table[x=dist,y=measure, col sep=comma]{Data/Test1/OT/measure_Linfty/measure_Linfty_0_74.csv};
\addlegendentry{$\gamma = 0.74$}; 
\addplot+[color=magenta, mark = triangle]
table[x=dist,y=measure, col sep=comma]{Data/Test1/OT/measure_Linfty/measure_Linfty_0_78.csv};
\addlegendentry{$\gamma = 0.78$}; 
\addplot+[color=orange, mark = triangle]
table[x=dist,y=measure, col sep=comma]{Data/Test1/OT/measure_Linfty/measure_Linfty_0_81.csv};
...
\subfloat[{\label{fig:crack_h_ref-infty}} OT Mesh with $\leb{\infty}$ a-posteriori bound]{
    \includegraphics[scale=0.15]{Images/Test2/h-adaptive/u_infty.png}
  }
\subfloat[{\label{fig:crack_h_ref-0.0}} OT Mesh with $\leb{2}$ a-posteriori bound ($\beta = 0.0$)]{
    \includegraphics[scale=0.15]{Images/Test2/h-adaptive/u_0_0.png}
  }
\subfloat[{\label{fig:crack_h_ref-0.99}} OT Mesh with $\leb{2}$ a-posteriori bound ($\beta = 0.99$)]{
    \includegraphics[scale=0.15]{Images/Test2/h-adaptive/u_0_99.png}
  }\\
\centering
\subfloat[{\label{fig:Winslow posteriori}} Mesh obtained with Winslow' method]{
    \includegraphics[scale=0.23]{Images/Test2/r-adaptive/u_0_99_11862.png}
}
...
\caption{Asymptotic convergence rates for different adaptive strategies. Note that the rate of convergence is optimal for h-refinement and the OT strategy ($\gamma = 0.6 / 0.75$ in $\leb{2} / \leb{\infty}$) norm.}
\end{figure}
...
%\draw[ultra thick,black] (0.002,0.007) -- (0.008,0.001);   
\addplot+[color=black, mark = triangle]
table[x=dist,y=measure, col sep=comma]{Data/Test2/OT/measure_L2/measure_L2_0_47.csv};
\addlegendentry{$\gamma = 0.47$}; 
\addplot+[color=red, mark = triangle]
table[x=dist,y=measure, col sep=comma]{Data/Test2/OT/measure_L2/measure_L2_0_53.csv};
\addlegendentry{$\gamma = 0.53$}; 
\addplot+[color=green, mark = triangle]
table[x=dist,y=measure, col sep=comma]{Data/Test2/OT/measure_L2/measure_L2_0_59.csv};
\addlegendentry{$\gamma = 0.59$}; 
%\addplot+[color=blue, mark = triangle]
%table[x=dist,y=measure, col sep=comma]{Data/Test2/OT/measure_L2/measure_L2_0_65.csv};
%\addlegendentry{$\gamma = 0.65$}; 
\addplot+[color=cyan, mark = triangle]
table[x=dist,y=measure, col sep=comma]{Data/Test2/OT/measure_L2/measure_L2_0_71.csv};
\addlegendentry{$\gamma = 0.71$}; 
\addplot+[color=magenta, mark = triangle]
table[x=dist,y=measure, col sep=comma]{Data/Test2/OT/measure_L2/measure_L2_0_78.csv};
\addlegendentry{$\gamma = 0.78$}; 
\addplot+[color=orange, mark = triangle]
table[x=dist,y=measure, col sep=comma]{Data/Test2/OT/measure_L2/measure_L2_0_84.csv};
\addlegendentry{$\gamma = 0.84$}; 
\end{axis}
\end{tikzpicture}
}\\
...
\begin{tikzpicture}
  \begin{axis}[
    ...
    % \draw[ultra thick,black] (0.002,0.005) -- (0.008,0.00047);
\addplot+[color=black, mark = triangle]
table[x=dist,y=measure, col sep=comma]{Data/Test2/OT/measure_Linfty/measure_Linfty_0_62.csv};
\addlegendentry{$\gamma = 0.62$}; 
\addplot+[color=red, mark = triangle]
table[x=dist,y=measure, col sep=comma]{Data/Test2/OT/measure_Linfty/measure_Linfty_0_65.csv};
\addlegendentry{$\gamma = 0.65$}; 
\addplot+[color=green, mark = triangle]
table[x=dist,y=measure, col sep=comma]{Data/Test2/OT/measure_Linfty/measure_Linfty_0_68.csv};
\addlegendentry{$\gamma = 0.68$}; 
%\addplot+[color=blue, mark = triangle]
%table[x=dist,y=measure, col sep=comma]{Data/Test2/OT/measure_Linfty/measure_Linfty_0_71.csv};
%\addlegendentry{$\gamma = 0.71$}; 
\addplot+[color=cyan, mark = triangle]
table[x=dist,y=measure, col sep=comma]{Data/Test2/OT/measure_Linfty/measure_Linfty_0_74.csv};
\addlegendentry{$\gamma = 0.75$}; 
\addplot+[color=magenta, mark = triangle]
table[x=dist,y=measure, col sep=comma]{Data/Test2/OT/measure_Linfty/measure_Linfty_0_81.csv};
\addlegendentry{$\gamma = 0.81$}; 
\addplot+[color=orange, mark = triangle]
table[x=dist,y=measure, col sep=comma]{Data/Test2/OT/measure_Linfty/measure_Linfty_0_84.csv};
\addlegendentry{$\gamma = 0.84$}; 
\end{axis}
\end{tikzpicture}
}
...
This shows that as $\gamma$, and consequently the interior angle $\omega$ increases, the mesh Quality deteriorates. This is also qualitatively suggested by looking at the OT mesh produced for the L-shaped and the crack domain (Figure  ).


\begin{figure}[h!]\label{tikz:Skewness_vs_gamma}
\centering
\subfloat[\label{Q_vs_gamma} Skewness for OT mesh as a function of $\gamma$]{ 
\begin{tikzpicture}
  \begin{axis}[
      cycle list/Dark2,
      thick,
...
\begin{tikzpicture}
  \begin{axis}[
      ...
      width=0.4\linewidth,
      %no marks,
      legend style={at={(1.5,1.0)},anchor=north east},
    ]
\addplot+[color = black,  mark = triangle]
...
\caption{Here we show that the dependence of the Skewness measure on $\gamma$ computed numerically fits the theoretical formula derived in Eq. \ref{eq:Q(gamma)}. The right plot also shows that the value of $Q(\gamma)$ is independendent on the dimension of the FE space.}
\end{figure}
...
\begin{equation}
\rho = \left( 1 + \frac{1}{\delta}\Norm{D^{l}u}_{l^{p}}\right)^{\frac{dq}{d + q(l-m)}}
