...
\end{axis}
\end{tikzpicture}
\caption{a.posteriori indicator function as function of r (A=2.19 $\times 10^{-5}$, B=4.9$\times 10^{-6}$, ,C=-1.22).}
\end{figure}
...
\end{axis}
\end{tikzpicture}
\caption{Values of the a-posteriori monitor function near the re-entrant corner (C = -1.49).}
\end{figure}
...
\addplot+[color=green, mark = diamond]
table[x=dof,y=error, col sep=comma]{Data/Test2/OT/a_priori/errorL2_0.5.csv};
\addlegendentry{OT a-priori ($\gamma = 0.5$)};
\addplot+[color=orange, mark = square]
