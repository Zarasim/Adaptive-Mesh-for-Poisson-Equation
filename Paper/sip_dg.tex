...
\abstract{In this paper, we investigate the properties of h- and r-adaptive mesh strategies for the numerical solution
  of the Poisson equation, discretised with the discontinuous Galerkin method, on non-convex domains. To drive the mesh adaptation, we derive a \textit{novel a-posteriori estimator} by using the theoretical framework of weighted Sobolev spaces and the duality argument. A  $\leb{\infty}$ a-posteriori bound will be also used in the numerical experiments, which will show the optimal convergence property for both the estimators.  Concerning the r-adaptive methods, we will evidence that a parameter-dependent Optimal Transport strategy is able to outperform the Winslow' MMPDE method, both in terms of accuracy and quality of the mesh. The parameter that numerically provides highest accuracy is justified by a theoretical argument. Another major contribution of this paper is the numerical evidence that the \textit{OT mesh with highest accuracy equidistributes the a-posteriori estimators in both norms}, hinting at the close relationship between r- and h- adaptation. Finally, we will prove and show numerically that the \textit{mesh skewness}, quality measure for OT meshes, does only depend on the re-entrant corner and not on the resolution of the mesh.
...
Using the \textit{jump} and \textit{avg} operator we can define the \text{mesh condition}  $q(\T{})$ to evaluate the quality of meshes, similarly as the shape regularity in \eqref{eq:shape_regularity}.  Moreover, under the condition 
...
\end{axis}
\end{tikzpicture}
\caption{Asymptotic convergence rates for different monitor functions $w$ \eqref{eq:monitor_gradient}-\eqref{eq:monitor_posteriori}.}
\end{figure}
...
\centering
\subfloat[{\label{fig:error_gamma_L2}}]{  
\begin{tikzpicture}
  \begin{axis}[
      cycle list/Dark2,
      thick,
...
\subfloat[{\label{fig:error_gamma_Linfty}}]{  
\begin{tikzpicture}
  \begin{axis}[
      cycle list/Dark2,
      thick,
...
We note that the process of h-refinement aims at equidistributing $\eta_{K}$ throughout each cell of the mesh. The a-posteriori measure can be then thought as a monitor function, as we have indeed done to define \eqref{eq:monitor_posteriori}.
...
\addplot+[color=black, mark = triangle]
table[x=dist,y=measure, col sep=comma]{Data/Test1/OT/measure_L2/measure_0_22.csv};
\addlegendentry{$\gamma = 0.22$}; 
\addplot+[color=red, mark = triangle]
table[x=dist,y=measure, col sep=comma]{Data/Test1/OT/measure_L2/measure_0_28.csv};
\addlegendentry{$\gamma = 0.28$}; 
\addplot+[color=green, mark = triangle]
table[x=dist,y=measure, col sep=comma]{Data/Test1/OT/measure_L2/measure_0_34.csv};
\addlegendentry{$\gamma = 0.34$}; 
\addplot+[color=blue, mark = triangle]
table[x=dist,y=measure, col sep=comma]{Data/Test1/OT/measure_L2/measure_0_4.csv};
...
\caption{(Left) Solution of the Poisson eq. \ref{eq:exact_solution_crack} via the WInslow' MMPDE. (Right) The computational domain $\W_{c}$ for the MMPDE a regular octagon.}
\end{figure}
...
\centering
\subfloat[{\label{fig:crack_error_L2}}]{  
\begin{tikzpicture}
  \begin{axis}[
      cycle list/Dark2,
      thick,
...
\subfloat[{\label{fig:crack_error_Linfty}}]{  
\begin{tikzpicture}
  \begin{axis}[
      cycle list/Dark2,
      thick,
...
\centering
\subfloat[\label{L2_norm_summary_crack}]{  
\begin{tikzpicture}
  \begin{axis}[
      cycle list/Dark2,
      thick,
...
\subfloat[\label{Linfty_norm_summary_crack}]{  
\begin{tikzpicture}
  \begin{axis}[
      cycle list/Dark2,
      thick,
...
\addlegendentry{$\gamma = 0.4$}; 
\addplot+[color=green, mark = triangle]
table[x=dist,y=measure, col sep=comma]{Data/Test2/OT/measure_L2/measure_0_47.csv};
...
For the $\leb{2}$ error norm the values of the parameters are: $d=2, k=1, l=2-\epsilon, p=2, m=0, q=2$.\\
