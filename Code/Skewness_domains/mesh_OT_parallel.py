...
def solveOT(idx_start,coords,gamma):
...
pool = mp.Pool() 
results = [pool.apply_async(solveOT, args = (idx_start_T[i],coords[idx_start_T[i]:idx_start_T[i+1],:],gamma)) for i in range(len(idx_start_T)-1)]
...
plot(mesh_OT)
#
#File('Paraview/OT_mesh/mesh_OT'+ str(N) + '_' + str(omega) + 'pi' + '.pvd') << mesh_OT
...
Q_scalar = np.max(Q.vector()[:])   
print(Q_scalar)
np.save('Q' + str(omega) + '.npy',Q_scalar)