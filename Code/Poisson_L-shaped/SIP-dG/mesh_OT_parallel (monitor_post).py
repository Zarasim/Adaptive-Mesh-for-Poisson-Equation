...
def solveOT(idx_start,coords,coeff):
    ...
    for i in range(coords.shape[0]):
        ...
        # solve OT equation 
        R = symbols('R')
        expr = 0.5*A*R**2 + B/(exponent+2)*R**(exponent+2) - 0.5*s**2
        sol = sympsolve(expr)
...
pool = mp.Pool() 
results = [pool.apply_async(solveOT, args = (idx_start_T[i],coords[idx_start_T[i]:idx_start_T[i+1],:],coeff)) for i in range(len(idx_start_T)-1)]
