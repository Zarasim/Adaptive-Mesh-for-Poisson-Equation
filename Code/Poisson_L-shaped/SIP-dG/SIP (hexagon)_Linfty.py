...
def monitor(mesh,u,beta,type_monitor):
    ...
    if type_monitor == 'gradient':
        ...
        # multiply by (ln(1/mince)**2) later 
        ...
        #1e3 for 6
        #1e3 for 120
        w.vector()[:] = np.sqrt(1 + 1e3*monitor.vector()[dof_P_to_C])
...
if output_file:    
    ...
    if type_monitor == 'a-posteriori':
        file_u = File('Paraview/L-shaped/r-adaptive/u_infty.pvd')
...
#ax.set_yscale('log')
#ax.set_xscale('log')    
    np.save('Data/r-adaptive/fit_data/monitor_infty_'+ str(N) +'.npy',w_1d)
    np.save('Data/r-adaptive/fit_data/dist_infty_' + str(N)  +'.npy',dist)
    
    dict = {'r': dist, 'w': w_1d}  
    df = pd.DataFrame(dict) 
    df.to_csv('Data/OT/a_priori/data_posteriori_infty_measure.csv',index=False) 
