...
for it,gamma in enumerate(gamma_vec): 
    ...
    if output:
        File_q = File('Paraview/q_'+ str(np.round(gamma, 2)) + '.pvd')
        File_mu = File('Paraview/mu_'+ str(np.round(gamma, 2)) + '.pvd')
...
#    sorted_array_2 = sorted_array_2[ind,:]
    ...
    for ref in range(n_ref): 
        ...
        q_vec[ref] = np.max(q.vector()[:])
        mu_vec[ref] = np.min(mu.vector()[:])   
...
#        File(string_mesh) << mesh_T
    ...
    np.save('Data/q_'+ str(np.round(gamma, 2)) +'.npy',q_vec)
    np.save('Data/mu_' + str(np.round(gamma, 2))  +'.npy',mu_vec)